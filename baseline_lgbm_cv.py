...
# Train final model on 70% training data
print("\nTraining Final LightGBM Model...")
logging.info("Starting final model training on 70% training data")
logging.info(f"LightGBM parameters: {lgb_params}")
...
experiment_results = {
    ...
    "cross_validation": {
        "mean_score": float(cv_mean_score),
        "std_score": float(cv_std_score),
        "mean_mape": float(cv_mean_mape),
...
logging.info("=" * 50)
logging.info(f"Cross-Validation Score: {cv_mean_score:.4f} ± {cv_std_score:.4f}")
